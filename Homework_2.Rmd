...
for (i in 1:11){
  ...
  mutate(nyc_df,
         !!as.symbol(col_name) :=
           as.character(pull(nyc_df,as.name(col_name))))
...
`r round(nrow(filter(nyc_df,!vending,entry))/nrow(filter(nyc_df,!vending)),3)` stations' entrance/exits without vending allow entrance.

...
1. staff hours including 3 types of information and etc. 

So the first step I did to the data is to make the name unique. Then I took the information in the route*: ffirst dealed with route 8 to 11 which not consistant with the other line format of data and put them into route_served variable using pivot_longer(). Followed with changing values in _entrance_ variable to boolean factors with if_else() function. And finally selecting the required variables using select().
...
* there's `r n_distinct(pull(nyc_df,"station_id"))` unique station (including names and line) in this data;
* Of `r n_distinct(pull(nyc_df,"station_id"))`, `r sum(pull(distinct(nyc_df,station_id,.keep_all = TRUE),'ada'))` are ADA compliant stations;
* Of `r n_distinct(pull(nyc_df,"station_id"))`, `r 1- round(sum(distinct(nyc_df,station_id,.keep_all = TRUE)$vending)/nrow(distinct(nyc_df,station_id,.keep_all = TRUE)),2)` station entrance/exits without vending allow entrance.

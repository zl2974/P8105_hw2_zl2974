...
`r nyc_df_tidy %>% distinct(station_name,line,.keep_all=TRUE) %>% filter(route_number=="A") %>% count()` serve route A.
...
which parties' candidates were in charge of White House. The columns *gov_dem,sen_dem,rep_dem*
 represent the number of governors, senators and representatives of Democrate Party and the others represent those of GOP. The data was record in the date of count on *date*. The data is in `r paste(nrow(read_five(1))," x ",ncol(read_five(1)))` dimention.
...
And use filter to the indicator columns to selected the informative rows( that is president's party of the record date).
